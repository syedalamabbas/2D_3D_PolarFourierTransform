function [ SphericalGrid ] = SinglePrecDirectBruteForce3D_DFT( inputVolume,  noOfAnglesTheta, noOfAnglesPhi)
%DIRECTBRUTEFORCE2D_DFT  Transforms the fucntion using the prescribed
% scheme as discussed in the paper by Syed Alam Abbas
%====================================================================
% Written on July 2nd, 2015 by Syed Alam Abbas.
%====================================================================

I = single(inputVolume);                       % 3D cube image, single precision
[sizeX, ~, ~] =  size(I);
N = sizeX -1;                          % N is even
K = noOfAnglesTheta;                   % K is also even it is for theta
M = noOfAnglesPhi;                     % M is also even it is for phi

SphericalGrid = zeros( K,M, N+1, 'single');      % Angle phi vs  Polar slices: No of angles vs. Radial data

deltaTheta = single(180/K);                    % Angular sampling rate theta
anglesTheta = single(0:deltaTheta:180-deltaTheta);

deltaPhi = single(180/M);                      % Angular sampling rate phi
anglesPhi = single(0:deltaPhi:180-deltaPhi);
...
%             if(1 == CheckPairsXZ(k,m,K,M))
                SphericalGrid(k,m,n)= SinglePrecDirectSinglePoint_3DDFT( I, desiredPoint );
%             end
        end   
        if (debug && k == 2 && m == 2) 
