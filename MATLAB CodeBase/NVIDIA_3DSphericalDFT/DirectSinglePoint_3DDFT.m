function [ DFT_Point ] = DirectSinglePoint_3DDFT( inputVolume, desiredPoint )
...
n = -N/2: N/2;
Map_x = exp(-1i*2*pi*u*n/(N+1));
Map_y = exp(-1i*2*pi*v*n/(N+1));
Map_z = exp(-1i*2*pi*w*n/(N+1));
...
for l = 1:N+1            % x index
    for m = 1: N+1       % y index
        for k =1:N+1     % z index
            DFT_Point = DFT_Point + I(l,m,k)*exp(-1i*2*pi*(u*n(l)+v*n(m)+w*n(k))/(N+1));   % Exactly according to the formula N^3 Complexity
        end
    end
end
end
