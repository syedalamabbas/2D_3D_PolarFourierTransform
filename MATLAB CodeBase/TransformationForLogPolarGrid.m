...
% title('\bf{Square grid & Polar grid & Log Polar grid}') 
...
for angle = angles 
    ...
    if(angle == deltaTheta || angle == 9*deltaTheta)
        plot(gridSpacing.*cosd(angle),gridSpacing.*sind(angle),'k','LineWidth', 1.2);
    end
end
hold off  

%% Plotting Log Polar grid

% figure,axis equal
% hold on

logSpacing = logspace(-N/2,N/2,6*N) ;
hold on 
for angle = angles 
    pause(delay) 
    h1= loglog(logSpacing*cosd(angle),logSpacing*sind(angle),'ko');
...
% plot(k *Newspacing.*cos(Newspacing),k *Newspacing.*sin(Newspacing),'k','LineWidth', 1.2);
%  
